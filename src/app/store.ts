...
import { taskMediaPatch, uniqueMediaVersions } from './media-result-state';
      ...
      if (normalizedStatus === 'success') {
        ...
        return {
          ...
          data: {
            ...
            ...(isUrl
              ? {
                  ...taskMediaPatch(prevData, resultUrl, task.id),
  ...
  try {
    ...
    setStore((state) => {
      const nodes = state.nodes.map((node) => {
        ...
        if (data.status === 'done' && data.taskId === task.id && data.queuedAfterTimeout !== true) return node;
        const hasHydratedTaskPreview = data.url === task.result_url;
        ...
        const preview = task.result_url
          ? {
              ...taskMediaPatch(data, task.result_url, task.id),
              assetStatus: task.status,
...
async function hydrateActiveTasks(
  ...
  const taskOwnerId = storageUserId;
  let tasks: TaskItem[];
  try {
  ...
  if (storageUserId !== taskOwnerId || !tasks || tasks.length === 0) return;
  ...
  const unsubscribe = useStore.subscribe(() => {
    if (storageUserId !== taskOwnerId) { unsubscribe(); return; }
...
export function stripHeavyFromNodeData(data: unknown): unknown {
  ...
  if (Array.isArray(out.versions)) {
    out.versions = uniqueMediaVersions(typeof out.url === 'string' ? out.url : '', out.versions as NodeVersion[])
...
export const useStore = create<AppState>()(persist((set, get) => ({
  ...
  switchBackendProject: async (id) => {
    ...
    if (state.activeBackendProjectId && state.canvasHydrated && state.backendProjects.find(project => project.id === state.activeBackendProjectId)?.my_role !== 'visitor') {
      try {
  ...
  runNode: async (nodeId, payload) => {
    ...
    const pendingNode = get().nodes.find(n => n.id === nodeId);
    const delaySeconds = useCanvasPreferences.getState().values.submitDelay;
    if (pendingNode && ['imageNode', 'panoramaNode', 'videoNode'].includes(pendingNode.type || '')) {
      const projectId = get().activeBackendProjectId;
      const ownerId = storageUserId;
      const originalParams = JSON.stringify(pendingNode.data.generationParams);
      if (delaySeconds > 0 && !pendingNode.data.taskId && pendingNode.data.status === 'running') {
        get().updateNodeData(nodeId, { status: undefined });
      }
      const proceed = await waitForCanvasSubmit(`${ownerId}:${projectId}:${nodeId}`, delaySeconds);
      const latest = get().nodes.find(n => n.id === nodeId);
      if (!proceed || !latest || storageUserId !== ownerId || get().activeBackendProjectId !== projectId || computeActiveProjectReadOnly(get())) return;
      if (JSON.stringify(latest.data.generationParams) !== originalParams) {
        toast.info('生成参数已变更，请确认后重新提交。'); return;
      }
    }
    ...
    set((snapshot) => {
      const nodes = snapshot.nodes.map((node) => node.id === nodeId
        ? { ...node, data: { ...node.data, status: 'running', generationOwnerId: storageUserId, error: undefined, lastGenerationError: undefined, lastGenerationFailedAt: undefined, taskId: undefined, queuedAfterTimeout: false, taskPhase: 'generating', assetSyncing: false, output: undefined, content: undefined, prompt: payload.prompt, resolvedPrompt, model: payload.model, runningStartedAt: startedAt } }
        : node);
      const projectStateById = syncActiveProjectState(snapshot, { nodes }).projectStateById;
      return {
        ...
        const nodes = snapshot.nodes.map((node) => {
          ...
          return {
            ...
            data: {
              ...
              ...(isUrlResult
                ? {
                    ...taskMediaPatch(prevData, persistedContent, result.task_id || `sync-${nodeId}-${startedAt}`),
