...
function ConfigDrawer({ config, open, onClose, onSaved }: DrawerProps) {
  ...
  const handleSave = async () => {
    ...
    const payload: ProviderConfigPayload = {
      ...
      submit_endpoint: apiSpec === "custom" ? submitEndpoint.trim() : "",
      query_endpoint: apiSpec === "custom" ? queryEndpoint.trim() : "",
  ...
  const showSubmitQueryFields = supportsCustomSubmitQueryEndpoints(apiSpec);
  const endpointPreview = getEndpointPreview(
    serviceType,
    apiSpec,
    submitEndpoint,
    queryEndpoint,
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5 space-y-5">
          ...
          <Field label="厂商" required>
            ...
            >
              ...
              {templates.map((t) => (
                <option key={t.vendor} value={t.vendor}>
                  {t.label}
                </option>
              ))}
          ...
          <Field
            label="API Key"
            hint={
              isEdit && config.api_key_set
                ? `已设置 (${config.api_key_hint})，留空不改`
                : undefined
            }
          >
            <Input
          ...
          {/* Submit / Query endpoints are only needed when the adapter profile
              cannot describe the upstream operation paths for us. */}
          ...
          <div className="rounded-lg border border-white/[0.08] bg-white/[0.03] px-3 py-2">
            <p className="text-xs font-medium text-neutral-400">端点预览</p>
          ...
          <Field
            label="模型列表"
            hint="选择预设厂商后自动填入，可编辑；多个用逗号或换行分隔"
          >
            <textarea
              value={modelListText}
          ...
          <Field
            label="默认模型"
            hint="该配置被选为「默认」时，生成故事/图片/视频将使用此处指定的模型。"
          >
            {modelListText.trim() ? (
              <select
                value={defaultModel}
              ...
              <Input
                disabled
                placeholder="请先填写上方模型列表"
                className="border-white/[0.06] bg-black/30 text-sm text-neutral-500 cursor-not-allowed"
              />
            )}
          </Field>
          ...
          <Field label="设为默认">
            ...
            >
              <span
                className={`absolute top-0.5 h-5 w-5 rounded-full bg-white shadow transition ${isDefault ? "left-[22px]" : "left-0.5"}`}
              />
            </button>
          </Field>
...
export function AdminModelCatalogPage() {
  ...
  const openEdit = (c: ProviderConfig) => {
    setEditConfig(c);
    setDrawerOpen(true);
  };
  const closeDrawer = () => {
        ...
        >
                ...
                filtered.map((c) => (
                  ...
                  >
                    <td className="px-4 py-3">
                      <Badge
                        className={
                          SERVICE_BADGE_STYLES[c.service_type] ??
                    ...
                    <td
                      className="px-4 py-3 max-w-[200px] truncate font-mono text-xs text-neutral-500"
                      title={c.base_url}
                    >
                      {c.base_url}
                    </td>
                    ...
                    <td className="px-4 py-3">
                      <span className="flex items-center gap-1.5 text-xs">
                        <span
                          className={`h-2 w-2 rounded-full ${STATUS_DOT[c.status] ?? "bg-neutral-600"}`}
                        />
          ...
          {!loading && filtered.length > 0 && (
            <div className="border-t border-white/[0.04] bg-white/[0.01] px-4 py-3 text-xs text-neutral-600">
              共 {filtered.length} 条
              {search ? `（共 ${configs.length} 条中过滤）` : ""}
              {enabledCount > 0 && (
                <span className="ml-3">· {enabledCount} 个已启用</span>
              )}
            </div>
          )}
        </div>
